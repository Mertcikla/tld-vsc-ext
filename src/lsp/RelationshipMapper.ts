...
import type { ResolvedArchitectureParserMode } from '../parsing/shared/types'
import type { TreeSitterQueryLoader } from '../parsing/treesitter/TreeSitterQueryLoader'
import {
  extractImportsWithRegex,
  resolveImportPath,
} from '../parsing/shared/imports'
import { detectGoModulePathFromText } from '../parsing/shared/externalLibraries'
...
export async function buildRelationshipGraph(
  ...
  loader: TreeSitterQueryLoader | null,
  config: {
  ...
  if (config.callHierarchyDepth > 0) {
    if (config.parserMode === 'treesitter' && loader) {
      await buildEdgesFromImports(symbols, symbolsByFile, adjacency, loader, token)
    } else if (config.parserMode === 'lsp') {
      await buildEdgesFromCallers(symbols, symbolsByFile, symbolsByRef, adjacency, loader, token)
    }
  }
...
async function buildEdgesFromImports(
  ...
  for (const sym of symbols) {
    ...
    try {
      ...
      const importPaths = rawImports.length > 0 ? rawImports : extractImportsWithRegex(text, sym.vscodeLangId)
...
/**
 * Extracts method/function calls from each file (via tree-sitter or LSP fallback)
 * and resolves them to symbols in the index to augment the dependency graph.
 */
async function buildEdgesFromCallers(
  symbols: ClassifiedSymbol[],
  symbolsByFile: Map<string, ClassifiedSymbol[]>,
  _symbolsByRef: Map<string, ClassifiedSymbol>,
  adjacency: Map<string, Set<string>>,
  loader: TreeSitterQueryLoader | null,
  token: vscode.CancellationToken,
): Promise<void> {
  ...
  let totalEdges = 0
  const processedFiles = new Set<string>()
  const symbolIndexByName = new Map<string, ClassifiedSymbol[]>()
  for (const sym of symbols) {
    const list = symbolIndexByName.get(sym.name) ?? []
    list.push(sym)
    symbolIndexByName.set(sym.name, list)
  }
  ...
  for (const sym of symbols) {
    ...
    try {
      ...
      let tsCallees = loader ? await loader.extractCalleeLines(text, sym.vscodeLangId) : []
      
      if (!tsCallees || tsCallees.length === 0) {
        // Fallback to LSP Call Hierarchy if tree-sitter fails or yields 0
        try {
          const lspSymbols = await vscode.commands.executeCommand<vscode.DocumentSymbol[]>(
            'vscode.executeDocumentSymbolProvider',
            fileUri
          )
          if (lspSymbols && lspSymbols.length > 0) {
            // Pick the first large enclosing symbol to ask for call hierarchy
            for (const lspSym of lspSymbols) {
              const items = await vscode.commands.executeCommand<vscode.CallHierarchyItem[]>(
                'vscode.prepareCallHierarchy',
                fileUri,
                lspSym.selectionRange.start
              )
          ...
          logger.trace('RelationshipMapper', 'LSP Call Hierarchy fallback failed', { error: String(e) })
        }
      }

      if (!tsCallees || tsCallees.length === 0) continue

      const fileSymbols = symbolsByFile.get(sym.filePath) ?? []
      if (!fileSymbols.length) continue
      
      // Sort fileSymbols by line number to make findOwningSymbol reliable
      const sortedFileSymbols = [...fileSymbols].sort((a, b) => a.startLine - b.startLine)

      for (const callee of tsCallees) {
        const callerSym = findOwningSymbol(sortedFileSymbols, callee.line)
        if (!callerSym) continue

        const callerRef = symbolRef(callerSym)
        const candidates = symbolIndexByName.get(callee.callee) ?? []
        for (const target of candidates) {
          const targetRef = symbolRef(target)
          if (callerRef === targetRef) continue
          
          if (!adjacency.has(callerRef)) adjacency.set(callerRef, new Set())
          if (!adjacency.get(callerRef)!.has(targetRef)) {
            adjacency.get(callerRef)!.add(targetRef)
            totalEdges++
          }
        }
      }
    } catch (e) {
      logger.debug('RelationshipMapper', 'callee edge extraction failed', {
        filePath: sym.filePath,
        error: String(e),
      })
    }
  ...
  logger.info('RelationshipMapper', 'caller-based edges extracted', {
    files: processedFiles.size,
    edges: totalEdges,
  })
}
...
async function detectGoModulePrefix(wsRoot: vscode.Uri): Promise<string | null> {
  try {
    ...
    const text = Buffer.from(bytes).toString('utf8')
    return detectGoModulePathFromText(text) ?? null
  } catch {
    return null
  }
}
